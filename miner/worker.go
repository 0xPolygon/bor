...
func (w *worker) commitWork(interrupt *atomic.Int32, noempty bool, timestamp int64) {
	...
	go w.prefetchFromPool(parent, throwaway, &genParams, stopPrefetch)
	w.buildAndCommitBlock(interrupt, noempty, &genParams)
	close(stopPrefetch)
}

// buildAndCommitBlock prepares work, fills transactions, and commits the block for sealing.
func (w *worker) buildAndCommitBlock(interrupt *atomic.Int32, noempty bool, genParams *generateParams) {
...
func (w *worker) prefetchFromPool(parent *types.Header, throwaway *state.StateDB, genParams *generateParams, stopCh <-chan struct{}) {
	...
	for {
		...
		// Check if we should stop before starting the loop iteration
		select {
		case <-stopCh:
			return
		default:
		...
		txs := newTransactionsByPriceAndNonce(signer, pendingTxs, header.BaseFee, &w.interruptBlockBuilding)
		...
		prefetcher.Prefetch(block, throwaway, vm.Config{}, &atomic.Bool{})
