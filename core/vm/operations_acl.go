...
func makeGasSStoreFuncPIP88(clearingRefund uint64) gasFunc {
	return func(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (GasCosts, error) {
		...
		if evm.chainRules.IsHampi && contract.Gas.RegularGas < access {
		...
		if original == current {
			...
			return GasCosts{RegularGas: cost + (params.SstoreResetGasEIP2200 - params.ColdSstoreCostPIP88)}, nil // write existing slot (2.1.2)
		}
...
func gasSLoadPIP88(evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (GasCosts, error) {
	loc := stack.peek()
	slot := common.Hash(loc.Bytes32())
	// Check slot presence in the access list
	if _, slotPresent := evm.StateDB.SlotInAccessList(contract.Address(), slot); !slotPresent {
		...
		// If he does afford it, we can skip checking the same thing later on, during execution
		evm.StateDB.AddSlotToAccessList(contract.Address(), slot)
		return GasCosts{RegularGas: params.ColdSloadCostPIP88}, nil
	}
