...
func (evm *EVM) runPrecompile(p PrecompiledContract, addr common.Address, input []byte, gas GasBudget) ([]byte, GasBudget, error) {
...
func (evm *EVM) runEcrecoverWithCache(stateDB StateDB, p PrecompiledContract, addr common.Address, input []byte, gas GasBudget, cache *sync.Map) ([]byte, GasBudget, error) {
...
// to 32KB on Bor networks and takes precedence; off Bor (or pre-Ahmedabad) the
// upstream helper applies, whose Amsterdam branch is dormant while AmsterdamBlock
// is nil.
func (evm *EVM) checkMaxCodeSize(size uint64) error {
	if evm.chainConfig.Bor != nil && evm.chainConfig.Bor.IsAhmedabad(evm.Context.BlockNumber) {
		if evm.chainRules.IsEIP158 && size > params.MaxCodeSizePostAhmedabad {
