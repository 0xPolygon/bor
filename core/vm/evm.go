...
func (evm *EVM) Call(caller common.Address, addr common.Address, input []byte, gas uint64, value *uint256.Int, interrupt *atomic.Bool) (ret []byte, leftOverGas uint64, err error) {
	...
	if isPrecompile {
		// Log modexp precompile invocation and completion (address 0x05)
		if addr == common.BytesToAddress([]byte{0x05}) {
			startGas := gas
			log.Info("MODEXP precompile called",
			...
			log.Info("MODEXP precompile finished",
				"block", evm.Context.BlockNumber,
				"txIndex", evm.StateDB.Inner().TxIndex(),
				"usedGas", usedGas,
				"error", err,
