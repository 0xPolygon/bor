...
// NewDatabase creates a state database with the provided data sources. The
// snapshot is only consulted by the merkle database; unified binary tries
// have no snapshot layer.
//
// Deprecated, please use NewMPTDatabase or NewUBTDatabase directly.
func NewDatabase(tdb *triedb.Database, snap *snapshot.Tree) Database {
	if tdb.IsUBT() {
		return NewUBTDatabase(tdb)
	}
	return NewMPTDatabase(tdb, snap)
}

// NewDatabaseForTesting is similar to NewDatabase, but it initializes the caching
// db by using an ephemeral memory db with default config for testing.
func NewDatabaseForTesting() *MPTDatabase {
	return NewMPTDatabase(triedb.NewDatabase(rawdb.NewMemoryDatabase(), nil), nil)
}
