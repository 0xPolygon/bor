...
// consult the WarmSnapshot before falling through to pathdb. Hits with a
// matching hash skip diff-layer/disk-layer/pebble work; misses are served by
// the underlying NodeReader unchanged, so correctness never depends on the
// snapshot's contents.
//
// A nil or empty snapshot is equivalent to New.
func NewWithCommitSnapshot(root common.Hash, db *CachingDB, snapshot *WarmSnapshot) (*StateDB, error) {
	if snapshot == nil || snapshot.Len() == 0 {
		return New(root, db)
	}
	return New(root, newSnapshotCommitStateDatabase(db, snapshot))
}
