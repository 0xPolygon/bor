...
func (dl *diskLayer) commit(bottom *diffLayer, force bool) (*diskLayer, error) {
	...
	if combined.full() || force || flush {
		...
		//
		// The copy below is shallow: []byte values share backing arrays
		// with the frozen buffer, so the cost is map structure only
		// (keys + slice headers), not data duplication.
