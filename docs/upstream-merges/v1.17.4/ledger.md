...
- **core/state slot-read tracking for empty storage (#33743, `e636e4e3c`)** — auto-merged;
  ...
  - **Superseded in the v1.17.3 part 3 cascade (#2345):** upstream #34776 (`6f02965aa`,
    batch 23) deleted the read outright. `GetCommittedState` now records every slot access in
    memory via `stateReadList.AddState`, with no trie walk, so the witness is identical on both
    sides of the fork and the skew this gate guarded against cannot arise at all. Took
...
| `core/state/dump.go` | #33102 | 2 | Took upstream's `acctIt.Hash().Hex()`; Bor's `it.Key` was stale once the iterator refactor auto-merged around it. **Operator-visible consequence, raised in review on #2340 and accepted rather than patched:** the same restructure changed how a `DumpConfig.Start` shorter than 32 bytes is read. Bor previously seeked with the raw bytes (`tr.NodeIterator(conf.Start)`), treating them as a high-order prefix; upstream converts first via `common.BytesToHash`, which left-pads — so a 2-byte start `0x1234` becomes `0x00…001234` and iteration begins near the bottom of the keyspace instead of roughly 7% in. Reaches `debug_accountRange` and `geth dump --start`. Paging is unaffected: `nextKey` is always a full 32-byte `acctIt.Hash().Bytes()`, so a client using the returned `Next` round-trips through `BytesToHash` unchanged, and only a hand-written short prefix differs — returning an earlier page rather than wrong account data. The file has zero divergence from upstream at `04e40995d`, so a local fix would be a Bor-only divergence that conflicts on every future batch touching it. To be reported upstream. |
...
Fifteen AI review comments across four PRs, resolving to fourteen distinct findings, plus two from human review of #2341, the #2341 deadlock having been reported by both bots. The one row below marked with a dash was found while classifying the others, not reported. The provenance check — diff the file
...
| #2341 | `receiptBufferLock` held across `dispatchRequest` | upstream has the same shape (`965bd6b6a` peer.go:409) | **fixed** `b246fba3d`, **superseded** `a2a83b9dd` (see the human-review row below) — a genuine deadlock: `dispatchRequest` waits on the peer's only dispatcher goroutine, which takes the same lock on its cancel path for any request. An unrelated header timeout wedges the peer for its lifetime. `RequestReceipts` already unlocks first, which settles it as an oversight |
...
| #2341 (human review) | Continuation becomes the pending entry after a cancel | **ours** — introduced by `b246fba3d`'s unlock-then-dispatch, which sent the continuation as an ordinary request | **fixed** `a2a83b9dd` by porting upstream's own fix, #35537 (`c57bbe417`, v1.17.6): a resend path that only sends while the original request is pending, plus a per-request `cleanup` callback. #35593 (`596096afe`) ported alongside. A fix to an AI finding introduced the next finding, and a human reviewer caught it with a repro — kept as `TestResendAfterCancelIsDropped` |
| #2341 (human review) | Truncated response can exceed `maxMessageSize` by the packet envelope | upstream has the same gap (`maxPacketSize - bytes`) | **fixed** `a2a83b9dd` — `receiptsPacketOverhead` reserved; the new size test fails 18 bytes over without it. Fixed despite being upstream's under the rule below: the serving side changes nothing a peer can observe except no longer being dropped |
| #2342 | Three `cmd/geth/snapshot.go` / `core/vm/eip7610.go` findings (nil `EIP158Block` dereference, missing-preimage exit code, chain-ID truncation) | upstream verbatim (#34718), unchanged in v1.17.5 | declined — none reachable on Bor: `EIP158Block` is 0 on every preset, Bor's chain IDs fit a uint64 and are absent from the EIP-7610 map, and the preimage path is an offline diagnostic that prints no partial list |
| #2342 | `DecodeRLP`'s BorWitness branch accepts an empty `Headers` list | **ours**, pre-existing on `develop` | **recorded, not fixed here** — every import path runs `ValidateWitnessPreState`, which rejects empty `Headers` before `Root()` is read, so it is not peer-reachable. A decode-time guard is POS-3726, targeting `develop`, since witness code is not changed inside a merge PR |
