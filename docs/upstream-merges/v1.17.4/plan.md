...
| Merge status    | **Complete — all 33 batches merged (2026-08-05).** `git merge-base --is-ancestor v1.17.4 HEAD` passes and all six milestone tags (`v1.16.9`, `v1.17.0`, `v1.17.1`, `v1.17.2`, `v1.17.3`, `v1.17.4`) are ancestors of the branch. Verification suite run and recorded in `ledger.md` (whole-tree tests, lint, vet in both tag modes, govulncheck, a 241-block real-mainnet gas-invariance replay, and two kurtosis devnets including a witness one). Milestone PR: **[#2346](https://github.com/0xPolygon/bor/pull/2346)** (draft, based on `ppatil-upstream-v1.17.3-part3`). |
...
| 33 | v1.17.4   | 7/7   | `36a7dc72e` | 4       | reflect.Pointer sweep (`all:`), **v1.17.4 release**                                             | merged (`f397229ee`) — **final batch; `git merge-base --is-ancestor v1.17.4 HEAD` confirms geth v1.17.4 is fully merged.** Smallest of the sync: 3 conflicts over two commits, 14 files, +20/−20. Both standing scans clean for the **fifth** batch running and trivially so — zero witness-mentioning hunks anywhere in the range and **not one hardfork-surface file touched**. `version/version.go`: the batch resolution kept bor's `Patch = 0`/`unstable` (batch-25 release-*tag* precedent, re-verified against all seven v1.17.x milestone tips), which also retired the stale "bump it in the M6 chores commit" plan item. **Subsequently reversed by operator decision on 2026-08-05, after this batch and the chores commit had been made: the file now reads `Patch = 4` / `Meta = "stable"`, i.e. identical constants to upstream's own v1.17.4 release commit.** The two commit messages that say otherwise cannot be amended without breaking every stacked PR; the ledger's batch-33 section is authoritative. #35176's two conflicts were blank-line-only (resolver-verified); one bor-only twin completed by hand (`internal/cli/server/service.go:270`, a dir upstream lacks — compiles either way since `const Ptr = Pointer`, but an `all:` sweep must not leave a file behind). **#35002 was a genuine no-op, verified not assumed**: bor already carried the fixed `minInterval - diff` from local commit `5410680b1` — the **fifth** upstream convergence onto bor. No regressions |
