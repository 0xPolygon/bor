...
- ```miner.disable-pending-block```: Disable the pending block creation loop on non block producer nodes. When set, RPC queries for 'pending' block will return an error (default: false)
