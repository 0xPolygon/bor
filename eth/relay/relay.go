...
// - Permanant rejection (i.e. tx which will always be rejected). As the tx has
// already passed the relay's local txpool validation, invalid txs are already
// filtered out.
// - Permanant rejection due to BP config (e.g. tx fee cap set by BP) which will
...
// will be hanlded by local re-submissions in BPs. If the tx still remains invalid
