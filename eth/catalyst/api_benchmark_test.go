...
func newBenchmarkBlobEnv(b *testing.B, blobCount int, version byte, fork benchFork) *benchmarkBlobEnv {
	...
	// Configure fork activations based on requested fork. bor's forks are
	// block-based, so the later forks are disabled by clearing their blocks.
	switch fork {
	case forkCancun:
		...
		for _, enc := range encodingTypes {
			b.Run(fmt.Sprintf("blobs=%d/enc=%s", blobCount, enc), func(b *testing.B) {
				...
				for i := 0; i < b.N; i++ {
					...
					result, err := env.api.NewPayloadV3(*execData, vhashes, &beaconRoot)
					if err != nil {
