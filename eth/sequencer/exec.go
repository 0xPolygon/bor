...
func newBlockEnv(chain *core.BlockChain, config *params.ChainConfig, statedb *state.StateDB, open *pb.BlockOpen, speculative map[uint64]common.Hash) *blockEnv {
	header := &types.Header{
	...
	if config.IsPrague(header.Number) {
...
func decodeSealedHeader(raw []byte) (*types.Header, error) {
	header := new(types.Header)
	if err := rlp.DecodeBytes(raw, header); err != nil {
		return nil, fmt.Errorf("decode sealed header: %w", err)
	}
