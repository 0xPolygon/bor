...
func (d *Downloader) concurrentFetch(queue typedQueue, beaconMode bool) error {
	...
	for {
			...
			for _, peer := range d.peers.AllPeers() {
				...
				if pending == nil && stale == nil {
					...
					if waited := time.Since(stale.Sent); waited > timeoutGracePeriod {
						...
						// stall the sync.
						peer.log.Warn("Peer stalling, dropping", "peer", peer.id, "waited", common.PrettyDuration(waited), "graceperiod", timeoutGracePeriod, "queue kind", queue.kind())
						d.dropPeer(peer.id)
					}
				}
			}
		...
		case <-timeout.C:
			...
			log.Debug("Downloader: request timed out, marking peer as stale", "peer", req.Peer, "queue kind", queue.kind(), "elapsed", common.PrettyDuration(time.Since(req.Sent)))
			...
			if fails > 2 {
				peer.log.Debug("Downloader: peer exceeded fail threshold, zeroing capacity", "peer", peer.id, "queue kind", queue.kind(), "fails", fails)
				...
				peer.log.Debug("Downloader: dropping peer on timeout", "peer", peer.id, "queue kind", queue.kind(), "fails", fails, "master", master)
				d.dropPeer(peer.id)
