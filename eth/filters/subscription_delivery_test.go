...
// A websocket client that subscribes to logs and then stops reading
...
func TestStalledSubscriberDoesNotBlockOthers(t *testing.T) {
	...
	httpServer := httptest.NewUnstartedServer(server.WebsocketHandler([]string{"*"}))
	// Bound queued TCP bytes so the close check measures connection closure,
	// not the time needed to drain a platform-dependent kernel send buffer.
	httpServer.Config.ConnState = func(conn net.Conn, state http.ConnState) {
		if state == http.StateNew {
			if tcp, ok := conn.(*net.TCPConn); ok {
				if err := tcp.SetWriteBuffer(4096); err != nil {
					t.Errorf("set server write buffer: %v", err)
				}
			}
		}
	}
	...
	send := func() {
		t.Helper()
		backend.logsFeed.Send([]*types.Log{{
