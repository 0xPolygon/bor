...
// TestShouldUseCompactWitness exercises the pure window/overlap decision helper.
func TestShouldUseCompactWitness(t *testing.T) {
	tests := []struct {
		name          string
		cacheWarm     bool
		parallel      bool
		...
		{
			name:          "before overlap uses compact",
			cacheWarm:     true,
			windowSize:    20,
			overlapSize:   10,
			blockNum:      9,
		...
		{
			name:          "non symmetric overlap start",
			cacheWarm:     true,
			windowSize:    30,
			overlapSize:   10,
			blockNum:      20, // windowStart + (windowSize - overlapSize)
			expectCompact: false,
		},
		{
			name:          "non symmetric before overlap stays compact",
			cacheWarm:     true,
			windowSize:    30,
			overlapSize:   10,
			blockNum:      10,
		...
		{
			name:          "parallel import always full",
			cacheWarm:     true,
			parallel:      true,
			windowSize:    20,
			overlapSize:   10,
		...
		{
			name:          "zero overlap behaves like no overlap",
			cacheWarm:     true,
			windowSize:    20,
			overlapSize:   0,
			blockNum:      15,
...
func TestCreateWitnessRequester(t *testing.T) {
	...
	requester := ethHandler.createWitnessRequester(100)
	if requester == nil {
