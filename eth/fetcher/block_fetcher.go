...
func (f *BlockFetcher) importHeaders(peer string, header *types.Header) {
	...
	go func() {
		...
		if err := f.verifyHeader(header); err != nil && err != consensus.ErrFutureBlock {
			log.Warn("Propagated header verification failed; dropping peer", "peer", peer, "number", header.Number, "hash", hash, "err", err)
			f.dropPeer(peer)
...
func (f *BlockFetcher) importBlocks(peer string, block *types.Block, witness *stateless.Witness) {
	...
	go func() {
		...
		default:
			...
			log.Warn("Propagated block verification failed; dropping peer", "peer", peer, "number", block.Number(), "hash", hash, "err", err)
			f.dropPeer(peer)
