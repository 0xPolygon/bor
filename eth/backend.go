...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	// START: Bor changes
	var privateTxStore *privatetx.PrivateTxStore
	if config.PrivateTxRelay {
		privateTxStore = privatetx.NewPrivateTxStore()
	}

	var privateTxStoreSetter privatetx.PrivateTxSetter
	if privateTxStore != nil {
		privateTxStoreSetter = privateTxStore
	}

	var privateTxStoreGetter privatetx.PrivateTxGetter
	if privateTxStore != nil {
		privateTxStoreGetter = privateTxStore
	}

	eth.APIBackend = &EthAPIBackend{stack.Config().ExtRPCEnabled(), stack.Config().AllowUnprotectedTxs, eth, nil, privateTxStoreSetter}
