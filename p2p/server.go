...
running:
	for {
		...
		case pd := <-srv.delpeer:
			...
			delete(peers, pd.ID())
			srv.log.Debug("Removing p2p peer", "peer", pd.ID(), "peercount", len(peers), "inbound", pd.Inbound(), "duration", d, "requested", pd.requested, "err", pd.err)
...
func (srv *Server) postHandshakeChecks(peers map[enode.ID]*Peer, inboundCount int, c *conn) error {
	...
	case !c.is(trustedConn) && len(peers) >= srv.MaxPeers:
		srv.log.Debug("Rejecting peer: too many peers", "peer", c.node.ID(), "peercount", len(peers), "maxpeers", srv.MaxPeers)
		return DiscTooManyPeers
	case !c.is(trustedConn) && c.is(inboundConn) && inboundCount >= srv.MaxInboundConns():
		srv.log.Debug("Rejecting peer: too many inbound", "peer", c.node.ID(), "inboundCount", inboundCount, "maxinbound", srv.MaxInboundConns())
		return DiscTooManyPeers
